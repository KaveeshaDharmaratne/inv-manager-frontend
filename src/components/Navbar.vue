...
<template>
    <nav class="flex-1 p-4 space-y-1">
        <div v-for="(item, index) in navStore.navItems" :key="index">
            <!-- Parent Menu Item -->
            <button @click="navStore.toggleExpand(item)"
                ...
                :class="isParentActive(item.children) ? 'text-zinc-900 dark:text-white font-semibold' : 'text-zinc-600 dark:text-zinc-400'">
